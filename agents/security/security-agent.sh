#!/usr/bin/env bash
# Unified Security Agent - Multi-Persona via CLI Tools
#
# Uses CLI tools (claude, codex, gemini, opencode) instead of direct API calls
# Behavior determined by symlink name:
#   security-claude.sh → claude CLI (Threat Modeler)
#   security-codex.sh → codex CLI (Vulnerability Scanner)
#   security-gemini.sh → gemini CLI (Penetration Tester)
#   security-opencode.sh → opencode CLI (Remediator)

set -euo pipefail

AGENT_ROOT="$(cd "$(dirname "$0")" && pwd)"
AGENT_MD="$AGENT_ROOT/security-agent.md"
...
case "$SCRIPT_NAME" in
    ...
    *)
        ...
        echo "Expected: security-claude.sh, security-codex.sh, security-gemini.sh, or security-opencode.sh" >&2
        exit 1
        ;;
esac
...
# Read input
INPUT=$(cat)
ROLE=$(echo "$INPUT" | jq -r '.role // "threat"')
...
extract_prompt() {
    ...
    /^### Persona: security-/ {
        if (in_role_section && $0 ~ "security-" persona) {
