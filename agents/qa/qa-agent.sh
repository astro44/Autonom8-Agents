...
AGENT_MD="$AGENT_ROOT/qa-agent.md"
...
# Extract role from input JSON (default to "functional" for QA agent)
ROLE=$(echo "$INPUT" | jq -r '.role // "functional"')

# Extract prompt from md file for this persona and role
extract_prompt() {
    local persona="$1"
    local role="$2"
    local qa_persona="qa-$persona"
    local role_upper=$(echo "$role" | tr '[:lower:]' '[:upper:]')

    awk -v persona="$qa_persona" -v role="$role" -v role_upper="$role_upper" '
    BEGIN {
        in_role_section = 0
