#!/usr/bin/env bash
# Unified OPS Agent - Multi-Persona via CLI Tools
#
# Uses CLI tools (claude, codex, gemini, opencode) instead of direct API calls
# Behavior determined by symlink name:
#   ops-monitor.sh → gemini CLI (System Monitor)
#   ops-triage.sh → codex CLI (Incident Triage)
#   ops-hotfix.sh → claude CLI (Automated Hotfix)
#   ops-rca.sh → codex CLI (Root Cause Analyst)
#   ops-perf.sh → claude CLI (Performance Optimizer)

set -euo pipefail

AGENT_ROOT="$(cd "$(dirname "$0")" && pwd)"
AGENT_MD="$AGENT_ROOT/ops-agent.md"
...
case "$SCRIPT_NAME" in
    ...
    ops-agent.sh)
        ...
        INPUT_PEEK=$(cat)
        PERSONA=$(echo "$INPUT_PEEK" | jq -r '.persona // "monitor"')

        case "$PERSONA" in
            monitor) CLI_TOOL="gemini" ;;
            triage|rca) CLI_TOOL="codex" ;;
            hotfix|perf) CLI_TOOL="claude" ;;
        esac
...
# Extract role from input JSON (default to "monitor" for Ops agent)
ROLE=$(echo "$INPUT" | jq -r '.role // "monitor"')

# Extract prompt from md file for this persona and role
extract_prompt() {
    local persona="$1"
    local role="$2"
    local ops_persona="ops-$persona"
    local role_upper=$(echo "$role" | tr '[:lower:]' '[:upper:]')

    awk -v persona="$ops_persona" -v role="$role" -v role_upper="$role_upper" '
    BEGIN {
        in_role_section = 0
