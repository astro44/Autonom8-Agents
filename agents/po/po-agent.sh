#!/usr/bin/env bash
# Unified PO (Product Owner) Agent - Multi-Persona via CLI Tools
#
# Uses CLI tools (claude, codex, gemini, opencode) instead of direct API calls
# Behavior determined by symlink name:
#   po-claude.sh → claude CLI (Product Strategist)
#   po-gemini.sh → gemini CLI (User Story Writer)
#   po-codex.sh → codex CLI (Sprint Planner)
#   po-opencode.sh → opencode CLI (Stakeholder Communicator)

set -euo pipefail

AGENT_ROOT="$(cd "$(dirname "$0")" && pwd)"
AGENT_MD="$AGENT_ROOT/po-agent.md"
...
case "$SCRIPT_NAME" in
    ...
    *)
        ...
        echo "Expected: po-claude.sh, po-gemini.sh, po-codex.sh, or po-opencode.sh" >&2
        exit 1
        ;;
esac
...
# Read input
INPUT=$(cat)
ROLE=$(echo "$INPUT" | jq -r '.role // "vision"')
...
extract_prompt() {
    ...
    /^### Persona: po-/ {
        if (in_role_section && $0 ~ "po-" persona) {
