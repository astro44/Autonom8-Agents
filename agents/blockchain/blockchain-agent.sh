#!/usr/bin/env bash
# Unified Blockchain Agent - Multi-Persona via CLI Tools
#
# Uses CLI tools (claude, codex, gemini, opencode) instead of direct API calls
# Behavior determined by symlink name:
#   blockchain-claude.sh → claude CLI (Threat Modeler)
#   blockchain-codex.sh → codex CLI (Vulnerability Scanner)
#   blockchain-gemini.sh → gemini CLI (Penetration Tester)
#   blockchain-opencode.sh → opencode CLI (Remediator)

set -euo pipefail

AGENT_ROOT="$(cd "$(dirname "$0")" && pwd)"
AGENT_MD="$AGENT_ROOT/blockchain-agent.md"
...
case "$SCRIPT_NAME" in
    ...
    blockchain-codex.sh)
        ...
        CLI_TOOL="codex"
        ;;
    blockchain-gemini.sh)
        PERSONA="gemini"
        CLI_TOOL="gemini"
        ;;
    blockchain-opencode.sh)
        PERSONA="opencode"
        CLI_TOOL="opencode"
        ;;
    blockchain-agent.sh)
    ...
    *)
        ...
        echo "Expected: blockchain-claude.sh, blockchain-codex.sh, blockchain-gemini.sh, or blockchain-opencode.sh" >&2
        exit 1
        ;;
esac
...
# Read input
INPUT=$(cat)
ROLE=$(echo "$INPUT" | jq -r '.role // "design"')
...
extract_prompt() {
    ...
    /^### Persona: blockchain-/ {
        if (in_role_section && $0 ~ "blockchain-" persona) {
