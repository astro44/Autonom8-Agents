#!/usr/bin/env bash
# Unified Data Agent - Multi-Persona via CLI Tools
#
# Uses CLI tools (claude, codex, gemini, opencode) instead of direct API calls
# Behavior determined by symlink name:
#   data-claude.sh → claude CLI (Threat Modeler)
#   data-codex.sh → codex CLI (Vulnerability Scanner)
#   data-gemini.sh → gemini CLI (Penetration Tester)
#   data-opencode.sh → opencode CLI (Remediator)

set -euo pipefail

AGENT_ROOT="$(cd "$(dirname "$0")" && pwd)"
AGENT_MD="$AGENT_ROOT/data-agent.md"
...
case "$SCRIPT_NAME" in
    ...
    *)
        ...
        echo "Expected: data-claude.sh, data-codex.sh, data-gemini.sh, or data-opencode.sh" >&2
        exit 1
        ;;
esac
...
extract_prompt() {
    ...
    /^### Persona: data-/ {
        if (in_role_section && $0 ~ "data-" persona) {
